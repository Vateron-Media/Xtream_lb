...
if ($argc) {
    ...
    $E8aba7524598d052fbd174c5a0e1cb52 = Db93ddef0764d97e767E03cF858aD457::C9F95DF7FAFb411701ded135f62E3bB5(MAIN_SERVER_ID, e2D73953a5089b9c7983838F61F23aCf::$StreamingServers[MAIN_SERVER_ID]["api_url_ip"] . "&action=getDiff", ["main_time" => time()], 5, 5);
    if ($E8aba7524598d052fbd174c5a0e1cb52 !== false && is_numeric($E8aba7524598d052fbd174c5a0e1cb52)) {
        ...
        $F30ccc8fd3427f97ca35bc6ba6610d01->query("UPDATE `streaming_servers` SET `diff_time_main` = '%d' WHERE `id` = '%d'", $E8aba7524598d052fbd174c5a0e1cb52, SERVER_ID);
    } else {
        $E8aba7524598d052fbd174c5a0e1cb52 = E2d73953a5089B9c7983838F61f23ACf::$StreamingServers[SERVER_ID]["diff_time_main"];
    }
    ...
    if (is_array($Beb85f0c05e519f48a14915b66ad155c) && $Beb85f0c05e519f48a14915b66ad155c["main_fetch"]) {
        ...
        $F30ccc8fd3427f97ca35bc6ba6610d01->query("UPDATE `streaming_servers` SET `server_hardware` = '%s',`last_check_ago` = '%d',`latency` = '%f',`whitelist_ips` = '%s' WHERE `id` = '%d'", json_encode($b9c2708ddebc9970f7c5ddc55aba1a07), time() + $E8aba7524598d052fbd174c5a0e1cb52, $Dec7b5ea9cc81b6c8a97f9e8c7a7b24a, json_encode($B46644daa5f44afb9d15520b844a615d), SERVER_ID);
        if (e2D73953a5089b9c7983838f61f23AcF::$StreamingServers[SERVER_ID]["status"] == 2) {
            $F30ccc8fd3427f97ca35bc6ba6610d01->query("UPDATE `streaming_servers` SET `status` = 1 WHERE `id` = '%d'", SERVER_ID);
        }
    }
